...
export default async function LeadsPage() {
  ...
  const sql=postgres(process.env.DATABASE_URL!, {max:1, prepare: false }); const lista=await sql`SELECT * FROM leads ORDER BY criado_em DESC`; await sql.end();
  return(<div className="p-5 sm:p-8"><h1 className="text-2xl font-bold mb-2">Leads</h1><p className="text-sm text-gray-500 mb-4">{lista.length} contatos</p>

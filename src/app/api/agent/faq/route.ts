...
export async function GET(request: NextRequest) {
  ...
  const sql=postgres(process.env.DATABASE_URL!, {max:1, prepare: false }); const lista=await sql`SELECT * FROM faq WHERE ativo=true AND visivel_agente=true ORDER BY ordem`;

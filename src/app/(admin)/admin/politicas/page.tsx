...
export default async function PoliticasPage({ searchParams }: { searchParams: Promise<{ erro?: string; ok?: string }> }) {
  ...
  const sql=postgres(process.env.DATABASE_URL!,{max:1}); const lista=await sql`SELECT * FROM politicas LIMIT 1`;
  const p=lista[0]; await sql.end();
  return(<div className="p-6"><h1 className="text-2xl font-bold mb-2">Politicas</h1>
